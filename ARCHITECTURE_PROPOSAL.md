...
- **Marco 5 — Governança** ✅: OpenLineage→Marquez end-to-end (sink + producer + dbt; `make up-lineage`), Soda quality checks (`governance/soda/` — OHLC invariant, gap de minuto, volume; `make soda-check`), SLO YAML + Prometheus alert rules (`governance/slo.yml` + `governance/prometheus_rules.yml`), freshness emitter standalone (`services/freshness_emitter.py`; `make freshness-check`), CI `governance-check` (valida YAML offline). 5 pilares: freshness ✅ volume ✅ distribution ✅ schema ✅ lineage ✅.
- **Marco 6 — Reprocessamento (Kappa)** ✅: `services/kappa_replay.py` (BoundedConsumer para ao HWM, grupo descartável, `--from-beginning | --from-timestamp ISO | --pipeline trades|candles|all | --dry-run`); `services/backtest.py` (time-travel Iceberg → DuckDB, analytics + invariantes, `--as-of | --snapshot-id | --list-snapshots`); `libs/pulso-storage/src/pulso_storage/replay.py` (`is_caught_up` puro + `ReplaySummary`); `RUNBOOK_KAPPA.md` (replay, backfill, migração ksqlDB, rollback via snapshot, checklist); `make replay` / `make backtest`. Testes offline: 14 casos cobrindo `is_caught_up`, throughput, reproducibilidade do time-travel e idempotência do replay (cerne do Kappa).
- **Marco 7 — Eixo B (LLM)** ✅: `services/anomaly_detector.py` — rolling window por símbolo, detecção de PRICE_SPIKE/VOLUME_SPIKE/VOLATILITY_SPIKE (z-score + % change), publica em `events.anomaly` (Avro, Schema Registry), /metrics :8003; `services/llm_explainer.py` — consome `events.anomaly`, busca notícias (RSS best-effort), chama Claude API (Haiku, **prompt caching** no system prompt), armazena em DuckDB `anomaly_explanations`, /metrics :8004; `contracts/anomaly.avsc` (BACKWARD enforçado em CI); `pulso_serve.anomaly_store` compartilhado entre explainer e API; API `GET /api/anomalies` (filter por symbol, limit); 24 testes offline cobrem rolling window, detecção, AnomalyStore e funções puras do LLM explainer; `make anomaly-detector` / `make llm-explainer`. Requer `PULSO_ANTHROPIC_API_KEY`.
- **Marco 8 — Cloud (GCP)** ✅: Terraform em `infra/terraform/` — GCS (lakehouse Iceberg + DuckDB anomalias montado via GCS volume), Artifact Registry, Cloud SQL Postgres (catálogo JDBC Iceberg), Secret Manager, IAM com service accounts de menor privilégio, Workload Identity Federation (GitHub Actions sem chave longa), 5 serviços Cloud Run (`pulso-ingest`, `pulso-sink`, `pulso-anomaly-detector`, `pulso-llm-explainer`, `pulso-serve`); `Dockerfile` multi-stage uv workspace (imagem única, CMD sobrescrito por serviço); `.github/workflows/deploy.yml` (build → push Artifact Registry → update Cloud Run, disparado após CI); `infra/COST_ANALYSIS.md` (~$40/mês); GCS FileIO (`gs://`) no catálogo PyIceberg via ADC; PORT env var no `pulso-serve` (Cloud Run). `make tf-init | tf-plan | tf-apply | docker-build | docker-push`.
